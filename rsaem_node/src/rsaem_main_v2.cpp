...
//////////////////////
  ...
  ************************************************************/
  update_timer_ = this->create_wall_timer(30ms, std::bind(&RSaemBot::joint_state_enc_callback, this)); 
...
/*
void RSaemBot::initJointStates(void)
{
  ...
  joint_states_.name[0] = "left_wheel_hinge";
  joint_states_.name[1] = "right_wheel_hinge";
  joint_states_.header.frame_id = "base_link";

}
*/

void RSaemBot::update_joint_state(int32_t LeftEncoder, int32_t RightEncoder, const rclcpp::Duration & duration)
{
  //static std::array<double, 2> last_joint_positions = {0.0f, 0.0f};
  //printf("[update_joint_state]\n");
  int32_t deltaLeft, deltaRight;
  
	deltaLeft = LeftEncoder - last_joint_positions[0];
    deltaRight = RightEncoder - last_joint_positions[1];
    
	if(abs(deltaLeft) > 3000)
		deltaLeft = last_joint_positions[0];
	if(abs(deltaRight) > 3000)
		deltaRight = last_joint_positions[1];    
      
  diff_joint_positions_[0] = deltaLeft;
  diff_joint_positions_[1] = deltaRight;
  
  double step_time = duration.seconds();
  
  v_left = (deltaLeft * DistancePerCount) / step_time;
  v_right = (deltaRight * DistancePerCount) / step_time;
  printf("v_left: %f v_right: %f\n", v_left, v_right);
  
  position_[0] = deltaLeft;
  position_[1] = deltaRight;
    
  last_joint_positions[0] = LeftEncoder;
  last_joint_positions[1] = RightEncoder;
  
  last_velocity[0] = v_left;
  last_velocity[1] = v_right; 
}

/*******************************************************************************
* Update the joint states 
*******************************************************************************/
void RSaemBot::publish_joint_state(void)
{
  //ROS_INFO("publish_joint_state");
  /*
  joint_states_.position[0] = last_rad[LEFT];
  joint_states_.position[1] = last_rad[RIGHT];
  joint_states_.velocity[0] = last_velocity[LEFT];
  joint_states_.velocity[1] = last_velocity[RIGHT];*/
  ...
  joint_states_->position.push_back(TICK2RAD * last_diff_position_[0]); // left
  joint_states_->position.push_back(TICK2RAD * last_diff_position_[1]); // right

  joint_states_->velocity.push_back(RPM_TO_MS * last_velocity[0]); //TODO RPM_TO_MS?
  joint_states_->velocity.push_back(RPM_TO_MS * last_velocity[1]); //TODO RPM_TO_MS?

  // msg->effort.push_back(current[0]);
  // msg->effort.push_back(current[1]);

  last_diff_position_[0] += (position_[0] - last_position_[0]); //TODO last_diff_position? last_position?
  last_diff_position_[1] += (position_[1] - last_position_[1]); //TODO //CHECK

  last_position_ = position_; //TODO //CHECK
...
void RSaemBot::joint_state_enc_callback(void)
{
  ...
  //RCLCPP_INFO(this->get_logger(), "current_time.nanoseconds():%d", current_time.nanoseconds() );
  //RCLCPP_INFO(this->get_logger(), "last_time.nanoseconds():%d", last_time.nanoseconds() );
  ...
  update_joint_state(LeftEncoder, RightEncoder, duration);
...
///////////////////
  // joint states
  updateJointStates();
  joint_states_.header.stamp = stamp_now;
...
/*
void RSaemBot::update_joint_state()
{
...
void RSaemBot::update_tf(geometry_msgs::msg::TransformStamped & odom_tf)
{
...
{
      printf("command_velocity_callback\n");
      float speedCmd = 0;
      float headingCmd0 = 0;
